...
const { backupFolder, serverName } = require("./minecraft/config.json");
...
for (const file of commandFiles) {
    const command = require(`./minecraft/${file}`);
    mcCommandsList.push(command)
}

module.exports = {
    name: 'minecraft',    //nom de la commande, en accord avec son nom de fichier
    aliases: ['mc'],   //alias de la commande, pour l'appeler (sur discord$) de plusieurs manieres
    description: 'Gere le serveur minecraft',
    usage: '$minecraft <command>',      //syntaxe, affichée si la commande est saisie mais incorrecte et dans l'help
    ...
    \`backup\`: lance un backup de la map du serveur\n\
    \`mods\`: affiche la liste des mods recommandés',    //une documentation supplementaire, dans le $help commande
	execute(message, args) {    //execution de la commande, ici une template embed
        ...
        message.channel.send("la commande n'est pas reconnue parmis les commandes minecraft \nverifie le \`$help minecraft\`")
